...
#   src/pzpdir/pzpdir.h            -> pzpdir/pzpdir.h              (the API the DataLoader includes)
#   src/pzpdir/pzpdir_internal.h   -> pzpdir/pzpdir_internal.h     (shared by the library's files)
#   src/pzpdir/pzpdir_*.c          -> pzpdir/pzpdir_*.c            (the library, without the CLI)
...
#
# The DataLoader's makeLibrary.sh / Makefile build pzpdir/libpzpdir.so from these files and link
# libDataLoader.so against it (rpath $ORIGIN/pzpdir). Vendored files that are no longer part of the set
# (e.g. the old pzpdir.c + pzpdir_*.inc.c single-file layout) are removed.
#
...
src/pzpdir/pzpdir_internal.h:pzpdir/pzpdir_internal.h
src/pzpdir/pzpdir_unicode.h:pzpdir/pzpdir_unicode.h
$(cd "$HERE" && for p in src/pzpdir/pzpdir_*.c; do [ "${p##*/}" = pzpdir_cli.c ] || echo "$p:pzpdir/${p##*/}"; done)
src/pzpdir/third_party/xxhash.h:pzpdir/third_party/xxhash.h"
...
# Stale: vendored library files that are no longer in the set
STALE=""
for f in "$DL"/pzpdir/pzpdir*.c "$DL"/pzpdir/pzpdir*.h; do
  [ -e "$f" ] || continue
  echo "$FILES" | grep -q ":pzpdir/${f##*/}$" && continue
  echo "stale    pzpdir/${f##*/}"; STALE="$STALE $f"
  if [ $INGIT = 0 ] || ! git -C "$DL" diff --quiet HEAD -- "$f"; then LOCAL="$LOCAL pzpdir/${f##*/}"; fi
done

if [ -z "$CHANGED" ] && [ -z "$STALE" ]; then echo -e "\033[32mDataLoader is up to date\033[0m"; exit 0; fi
...
for f in $STALE; do rm -f "$f" && echo -e "\033[32mremoved\033[0m  pzpdir/${f##*/}"; done
for m in $CHANGED; do
  SRC="$HERE/${m%%:*}"; DST="$DL/${m#*:}"
...
# Build check: pzpdir under the DataLoader's flag sets, built as libpzpdir.so, the DataLoader's sources
# ( incl. codecs/pzpInput.c and the new codecs/pzp.h ) linked against it, then the pzp CLI exactly as
# makeLibrary.sh builds it.
FAIL=0
...
if [ $FAIL = 0 ]; then echo -e "\033[32mvendoring done\033[0m (not committed; rebuild libpzpdir.so + libDataLoader.so with make / makeLibrary.sh)"; exit 0; fi
