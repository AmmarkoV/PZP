...
- **v0.4 revision 12 (2026-09-23): word index**, implemented in phase 8 (`src/pzpdir/`, library 0.11).
  - **What:** an index-resident **word index** over a table's text column (§3.7, §4.10; decision D20): per word its
    records / count / postings, and per record its words. It is built at pack time (`@words TABLE.COLUMN`,
    `pzpd_writer_words()`), because archives are written once and read many times, and a concept-balancing loss or
    sampler needs these statistics at every start.
  - **Words, not token IDs:** words are stored as strings, word ids are handle-local, and token IDs stay the loader's
    job (D16 unchanged).
  - **Sources:** with a declared source column (`@words descriptions.text source`), one sub-index per source value plus
    a stored merged sub-index (D21).
  - **Synonyms:** explicit merge rules (`dogs → dog`) in a reserved `synonyms` global table, applied at open (canonical
    view), so editing them never rebuilds the index.
  - **Format:** sections `PZPDSECT` kind 14 (shard) and 15 (manifest vocabulary), plus a 4-slot directory in the
    superblock and manifest header.
  - **API and tools:** `pzpd_words_*`, Python `Archive.words()` / `word_sources()`, CLI `words` / `reindex`, phase 8.
  - **From the implementation:**
    - **Header extension:** the word directory, `words_checksum` and `ext_checksum` sit **after** `sb_checksum`, and
      `index_checksum` doesn't cover word sections (§4.10). Older readers stay compatible; a damaged extension only
      disables the word indexes.
    - **Tokenizer tables:** `pzpdir_unicode.h`, generated by `scripts/gen_unicode_tables.py` from Python 3.12.3
      (Unicode 15.0.0). The C tokenizer equals Python's rule on every Unicode scalar value, on all COCO val2017
      captions and on 20 000 random strings. Invalid UTF-8 splits words like a non-word character.
    - **Edge rules:** rows with an empty source count in the merged sub-index only; a word is at most 65 535 bytes;
      a synonyms error names its row (e.g. `synonyms row 1: "dog" is both a canonical word and mapped`).
    - **API additions:** `pzpd_words_index()` (enumerate), `pzpd_words_arrays()` (whole vocabulary, zero-copy),
      `pzpd_tokenize()`, `pzpd_edit_words()`; Python `Archive.word_indexes()`, `tokenize()`, `reindex()`,
      `Writer.words()`; CLI `words --sources`, `reindex [TABLE.COLUMN [SOURCE_COLUMN]] [--drop]`, and `info` lists
      word indexes.
...
- **Table names** are ≤ 23 bytes and share the namespace with stream names. At most 16 tables per archive.
  The name `synonyms` is reserved for the word index's merge rules (§3.7).
...
### 3.7 Word index: word statistics for concept balancing (revision 12)

**Why.** An archive is written once and read by many training runs. A loss or
sampler that balances concepts needs, for every word, how many records contain
it and which ones, and for every record, its words. Examples: weighting a
sample by how rare its caption words are, or sampling "a record that mentions
*giraffe*". Deriving this from the caption text costs a tokenization pass over
every row at every start. Built once at pack time, it becomes an index-resident,
zero-parse `mmap`, like the tables.

**Words, not token IDs (D16 unchanged).** The index stores **words as strings**
with their statistics. Its word ids are handles that are valid only inside one
opened archive or collection handle. They are never token IDs, and nothing
outside the handle may depend on them. Mapping words to a model's token IDs
(vocabulary, pinned `index_to_word.json`, blacklist, embeddings) stays the
loader's job, as D16 says. The loader looks words up instead of re-tokenizing text.

**What is indexed.** A word index covers one `str` column of one record table,
declared at pack time with the list directive `@words TABLE.COLUMN [SOURCE_COLUMN]`
(e.g. `@words descriptions.text source`). An archive can have at most 4 word indexes.

**One index per source, plus a merged one (decision D21).** When a source
column is named (a `str` column of the same table, e.g. `source`), the word
index holds one **sub-index per distinct source value** (e.g. `deepseekvl2`,
`old`), built from that source's rows only. It also holds the **merged
sub-index**, built from every row whatever its source. Every sub-index has the
full structure below.
- **Per-source** sub-indexes answer questions such as "how rare is *giraffe* in
  the VLM captions", or "records whose original COCO caption mentions *dog*".
- **The merged** sub-index answers the same questions over all captions. A record
  with a matching row in any source is counted once.
- **Every access pattern reads stored data directly:** per word or per record,
  merged or for one source. The merged sub-index is **stored, not computed** from
  the per-source ones, and each source is read without scanning the others.

Without a source column, the index holds only the merged sub-index. The source
column itself is not a concept and is not indexed. Sub-indexes are named by their
source value; the merged one has no name (`source = NULL` in C, `None` in Python).

**Tokenizer v1.** It is stored as `tokenizer = 1` in the index, so a later rule
change is detected at open and never mixed silently. It is D16's word rule
without the punctuation symbols: `re.findall(r'\w+', text.lower())`, keeping only
all-ASCII tokens. It carries the same C parity requirement as §3.6: Python's
Unicode `\w` decides where non-ASCII characters split words.

**Contents.**
- **Per word:** its bytes; `records`, the number of records containing it
  (document frequency); `count`, its total occurrences; and its **postings**,
  the ascending ordinals of the records containing it.
- **Per record:** its distinct word ids, ascending. This forward list uses CSR
  like a record table.

**Synonyms: an explicit table.** A reserved global table `synonyms` with the
fixed schema `word:str, canonical:str` states merge rules explicitly:
...
- **An ordinary global table (§3.6):** written with `@global synonyms word:str canonical:str`
  and `@row synonyms dogs,dog` lines, listed by `export-table`, and changed by `replace-table synonyms`.
- **Rules are checked at pack and edit time** (`PZPD_E_FORMAT`, naming the row):
  - both sides are single words under tokenizer v1 (i.e. already lower-case ASCII);
  - a `word` appears at most once;
  - rules are one step: a `canonical` never appears as a `word`, so there are no chains or cycles.
- **Multi-word phrases are out of scope** for v1.

**Synonyms are applied at open, not at pack.** The index stores **surface words**,
the tokenizer's output before any synonym rule. Opening it in the
**canonical view** merges each canonical word with its surface forms:
- its `records` is the size of the **union** of their postings, so a record
  saying both "dog" and "dogs" counts once;
- its `count` is the sum;
- its postings are the merged, ascending union;
- a record's word list maps each word to its canonical form and removes duplicates.

This has three consequences:
- editing `synonyms` **never rebuilds** the index;
- both views are available: surface (to inspect the rules) and canonical (for training);
- a loader with its own synonym map can use the surface view and apply that map instead.

**Collections.** Word indexes merge over members by `(table, column)`, and
sub-indexes by source value (the merged sub-index with the merged one). Words
are matched by bytes, and postings use collection ordinals. A member without a
given source contributes no records to that source's sub-index. The tokenizer version
must agree, or opening that index fails with `PZPD_E_FORMAT`. A member without
the index contributes records with no words; the index handle reports its
coverage. The canonical view uses the **union** of the members' `synonyms`
rows. The same `word` with two different `canonical`s fails the canonical view
(`PZPD_E_FORMAT`), while the surface view still opens.

**Loss use (illustrative, not part of the format):**

```python
w = a.words("descriptions.text", canonical=True)
df = w.records_per_word                        # numpy, indexed by word id
weight_i = np.mean(1.0 / df[w.of_record(i)])   # rarer concepts weigh more
c = rng.integers(len(w)); i = rng.choice(w.records(c))   # concept-balanced sampling
vlm = a.words("descriptions.text", source="deepseekvl2", canonical=True)   # one caption source only
a.word_sources("descriptions.text")            # ['deepseekvl2', 'old']; None = merged
# the loader maps w.word(id) -> its own token id (D16); the archive never stores token ids
```

**Cost** (measured on COCO val2017 `descriptions.text` with tokenizer v1, 0.56 MB of caption text):

| Sub-index | Words | Record–word pairs | Bytes (vocabulary + postings + forward lists) |
|---|---|---|---|
| merged | 4 224 | 81 548 (16.3 per record) | ≈ 0.78 MB |
| `deepseekvl2` | 4 140 | 56 652 | ≈ 0.58 MB |
| `old` | 954 | 40 759 | ≈ 0.37 MB |
| **total** | | | **≈ 1.74 MB** (2.2 × the merged sub-index alone) |
...
### 4.10 Word index section (`PZPDSECT` kind 14; manifest vocabulary kind 15; revision 12)

**Directory: a header extension after `sb_checksum`.** The shard superblock and the manifest header each end
with a 272-byte extension placed **after** their `sb_checksum`:
- 4 slots of 64 B, `{table[24], column[24], u64 section_offset, u64 section_bytes}` (unused slots are zero);
  `section_offset` points at the section data;
- `u64 words_checksum`, the XXH64 of the word sections' data in directory order;
- `u64 ext_checksum`, the XXH64 of the extension bytes before it, or 0 when they are all zero.

`sb_checksum` stays at byte 1728 of the superblock (checked with `_Static_assert`), and `index_checksum` does not
cover word sections. So:
- an archive without word indexes is byte-identical to one written before revision 12;
- readers older than revision 12 ignore the extension and the kind-14/15 sections;
- a pre-revision-12 binary that edits an archive rewrites its superblocks without the extension and so drops its word
  indexes (their sections become dead space). They can be rebuilt with `reindex`.

A damaged extension (`ext_checksum` mismatch) never stops a shard from opening. Only its word indexes become
unusable (`PZPD_E_CHECKSUM`), table edits don't carry that directory forward, and `reindex` rebuilds the named index.

**Shard section** (kind 14), one per word index, holding all of its sub-indexes.
- **Section head** (64 B): `{u32 tokenizer, u32 flags, u32 subindex_count, u32 pad, char source_column[24],
  u64 names_offset, u64 names_bytes, u64 reserved}`. `source_column` is empty when there is no source column, and
  `names` holds the source values.
- **Sub-index heads** follow: `subindex_count` × 96 B
  `{u32 source_offset, u32 source_len, u64 records, u64 words, u64 postings, u64 vocab_offset, u64 post_index_offset,
  u64 post_offset, u64 fwd_index_offset, u64 fwd_offset, u64 heap_offset, u64 heap_bytes}`.
  - **Sub-index 0** is always the merged one (`source_len` = 0).
  - **Sub-indexes 1..k** are one per source value present in the shard, sorted by source bytes; k ≤ 255.
  - A shard that has no rows of some source has no sub-index for it.
- **Offsets** are relative to the section data, and each part is 8-byte aligned.

Each sub-index has these parts:

| Part | Layout | Notes |
|---|---|---|
| vocabulary | `words` × 16 B `{u32 heap_offset, u16 len, u16 pad, u32 records, u32 count}` | sorted by word bytes (`memcmp`), so lookup is a binary search; the index in this array is the shard-local word id |
| postings index | (`words` + 1) × `u32` | CSR over the postings |
| postings | `postings` × `u32` shard-local ordinals | ascending within each word |
| forward index | (`records` + 1) × `u32` | CSR over the forward list; `records` = the shard's record count |
| forward list | `postings` × `u32` shard-local word ids | ascending within each record; it is the transpose of the postings, so both have `postings` entries |
| heap | word bytes | no NUL |

- **Checksum:** each section carries its own XXH64 (section header), and `words_checksum` covers them all; see the
  directory above.
- **Rows with an empty source value** count in the merged sub-index only (an empty name is reserved for it).
- **A word is at most 65 535 bytes** (`u16 len`); packing a longer one fails.
- **Manifest section** (kind 15): for each sub-index (merged first, then the union of all shards' source values,
  sorted), the **merged vocabulary** of all shards. It has the same 64-byte head, then `subindex_count` × 48 B
  `{u32 source_offset, u32 source_len, u64 words, u64 vocab_offset, u64 heap_offset, u64 heap_bytes, u64 reserved}`,
  the source names, and per sub-index `words` × 24 B `{u32 heap_offset, u16 len, u16 pad, u64 records, u64 count}`
  plus its word heap.
  - Words are sorted by bytes, each with total `records` and `count`. Shards hold disjoint records, so these totals are exact sums.
  - Vocabulary and document frequencies are therefore available without opening any shard (loss weights, the viewer's word list).
  - Postings and forward lists come from the shard sections, which open lazily.
  - The merge from shard-local to handle word ids is built at open by one linear merge of sorted vocabularies.
- **Validation is lazy**, as for tables (revision 4). At open only the head, the part bounds, `post_index[words] == postings`
...
// ---- word index (§3.7, revision 12): index-resident, zero-copy per shard ----
// A handle holds one view (surface or canonical) of one word index; its word ids are valid only for that handle.
int        pzpd_words_open(pzpd *a, const char *table, const char *column,
                           const char *source, size_t source_len, unsigned flags, pzpd_words **out);
           // source = NULL: the merged sub-index; otherwise one source value's sub-index (D21)
           // flags: PZPD_WORDS_CANONICAL (apply the `synonyms` global table); 0 = surface words
size_t     pzpd_words_sources(pzpd *a, const char *table, const char *column,
                              const char **names, size_t *lens, size_t max);   // source values; returns the total
void       pzpd_words_close(pzpd_words *w);
uint32_t   pzpd_words_count(const pzpd_words *w);                              // vocabulary size
const char*pzpd_words_word(const pzpd_words *w, uint32_t id, size_t *len);    // word bytes
int64_t    pzpd_words_find(const pzpd_words *w, const char *word, size_t len); // id, or -1 if absent
int        pzpd_words_stats(const pzpd_words *w, uint32_t id, uint64_t *records, uint64_t *count);
           // manifest-resident: no shard opened
size_t     pzpd_words_records(pzpd_words *w, uint32_t id, uint64_t *ordinals, size_t max);
           // ascending ordinals of the records containing the word; returns the total (as pzpd_find_all)
size_t     pzpd_words_of_record(pzpd_words *w, uint64_t ordinal, uint32_t *ids, size_t max);
           // the record's word ids, ascending; returns the total
...
int        pzpd_words_index(pzpd *a, unsigned i, const char **table, const char **column, const char **source_column);
           // enumerate the handle's word indexes (over all members, first-seen order)
uint32_t   pzpd_words_arrays(const pzpd_words *w, const uint64_t **records, const uint64_t **count,
                             const char **heap, const uint64_t **offsets);   // the whole vocabulary, zero-copy
size_t     pzpd_tokenize(const char *text, size_t len, int (*emit)(const char *word, size_t len, void *user), void *user);
           // tokenizer v1, so loaders split text exactly like the index
int        pzpd_edit_words(const char *manifest, unsigned op, const char *table, const char *column, const char *source_column);
           // reindex: PZPD_EDIT_ADD / _REPLACE (rebuild, or add when missing) / _DROP; crash-safe like table edits
...
int  pzpd_writer_words(pzpd_writer *w, const char *table, const char *column, const char *source_column);
     // declare a word index before the first record (§3.7); rows of that column are tokenized as they are added;
     // source_column = NULL: merged sub-index only, else one sub-index per value of that column + the merged one
...
- **Word indexes (§3.7, revision 12)** are derived data and follow table edits:
  - `replace-table` of an indexed table **rebuilds** its word index sections, and `drop-table` drops them. Either
    way this happens in the same append + generation flip, so the table and its index never disagree.
  - `pzpdir reindex <a.pzpd> [TABLE.COLUMN]` adds or rebuilds an index on an existing archive (appended like
    `add-table`), e.g. for archives packed before revision 12.
  - Editing `synonyms` (`replace-table synonyms`) needs **no** reindex, because synonyms are applied at open.
  - Stream edits copy word index sections unchanged.
  - `salvage` does not recover word indexes, because they are re-derivable: run `reindex` after salvaging.
  - `rebuild-manifest` regenerates the merged vocabulary (kind 15) from the shard sections.
...
with pzpdir.open("coco_val2017.pzpd") as a:           # manifest, single shard, or collection file
    ...
    w = a.words("descriptions.text", canonical=True)   # word index (§3.7), merged sub-index; canonical = apply `synonyms`
    a.word_sources("descriptions.text")                # ['deepseekvl2', 'old']
    a.words("descriptions.text", source="old")         # one source's sub-index (D21)
    w.words; w.records_per_word; w.count_per_word      # list[str], numpy arrays by word id (no shard opened)
    w.find("dog"); w.records("dog")                    # word id; ascending ordinals (numpy u64)
    w.of_record(i)                                     # the record's word ids (numpy u32); w.word(id) -> str
    a.word_indexes()                                   # [('descriptions', 'text', 'source')]
pzpdir.tokenize("A Dog, café")                          # ['a', 'dog'] (tokenizer v1)
pzpdir.reindex("coco_val2017.pzpd", "descriptions.text", source_column="source")   # or drop=True; no spec: rebuild all
w.words("descriptions", "text", "source")               # Writer: declare a word index
...
pzpdir words <a.pzpd> [TABLE.COLUMN] [--source S | --sources] [--canonical] [--top N | --word W]
             # word  records  count  (by records, descending); --word W: the keys of the records containing W
             # --source S: one source's sub-index (default: merged); --sources: list the source values
pzpdir reindex <a.pzpd> [TABLE.COLUMN [SOURCE_COLUMN]] [--drop]
             # (re)build a word index from its table (§3.7), or drop it; no TABLE.COLUMN: rebuild every word index
...
| D20 | Word index (revision 12) | **Built at pack time** because archives are written once and read many times. It stores words as strings with records / count / postings per word and a sorted word list per record (§3.7, §4.10). It does not store token IDs (D16 unchanged). Tokenizer v1 is D16's word rule (`\w+`, lower-case, ASCII tokens only) and is versioned in the file. Synonyms live in an explicit `synonyms` global table (`word → canonical`, one step) and are **applied at open**, so synonym edits never rebuild the index. The index is rebuilt with its table on `replace-table` |

| D21 | Word index sources (revision 12) | When a source column is declared (`@words descriptions.text source`), the index stores **one sub-index per source value plus a stored merged sub-index** over all rows (§3.7, §4.10). Per-source, merged, and per-record lookups are all served directly from stored data, at ≈ 2.2 × the merged index's size on COCO (two sources) |
...
| 8 | Word index (§3.7, §4.10): `@words` (with source column), `pzpd_writer_words`, per-source + merged sub-indexes, sections kinds 14 / 15, `pzpd_words_*`, `synonyms` validation, canonical view, `words` / `reindex`, table-edit rebuild, Python `Archive.words()` | COCO val2017 `descriptions.text`: every record's word set, and every word's `records` / `count`, equal a Python reference (`re.findall(r'\w+', t.lower())`, ASCII tokens only) for the merged sub-index **and** for each source's sub-index (reference built from that source's rows); each word's merged postings equal the union of its per-source postings; the postings are exactly the transpose of the forward lists; the canonical view equals applying the `synonyms` rows in Python (union semantics: a record with "dog" and "dogs" counts once); a synonyms rule that chains, or repeats a `word`, is rejected with its row; `replace-table synonyms` leaves the shard word sections byte-identical; `replace-table descriptions` rebuilds them and kill -9 between append and flip leaves old or new, never mixed; a collection with overlapping vocabularies merges by word bytes; 1 M synthetic records: open + canonical merge < 1 s, and a word's postings are returned without touching table text; truncation / bit-flip fuzz of the sections never crashes (ASAN) |
